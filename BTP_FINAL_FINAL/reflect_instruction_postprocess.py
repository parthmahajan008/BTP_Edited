...
if __name__ == "__main__":
    ...
    for i, raw_data_i in enumerate(raw_data):
        ...
        ans_i = ori_data_i['answers'][0].strip()

        if isinstance(ori_data_i['ctxs'], list):
            ctx_i = "\n\n".join([f"[Title]: {c['title']}\n[Content]: {c['text']}" for c in data_i['ctxs']])
        else:
            ctx_i = str(ori_data_i['ctxs']).strip()
        ...
        temp_data['answer'] = ori_data_i['answers'][0]
    ...
    for i, data_i in enumerate(new_data):
        ...
        if (data_i['new_question'] == '') and (data_i['new_rationale'] == ''):
            ...
            temp_data['answer'] = data_i['answers'][0]
